...
import { buildUniqueAutoMapping, getSelectedFields } from "./import.mapping";
...
type Props = {
  ...
  analyzeFile: (file: File) => Promise<ImportAnalysis>;
  previewImport: (importId: string, mapping: Record<string, string | null>) => Promise<ImportPreview & { import: ImportRecord }>;
  startImport: (importId: string) => Promise<{ import: ImportRecord }>;
  fetchImport: (importId: string) => Promise<ImportRecord>;
  downloadErrorReport: (importId: string) => Promise<Blob>;
  downloadTemplate: (format: TemplateFormat) => Promise<Blob>;
  onClose: () => void;
  onCompleted: () => void;
};
...
const BulkImportModal = ({
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        const next = await fetchImport(importRecord.id);
        ...
        if (next.status === "completed" || next.status === "failed") {
          ...
          if (next.status === "completed") {
            toast.success(`${entityLabel} import completed.`);
            onCompleted();
          } else {
            toast.danger(next.last_error ?? `${entityLabel} import failed.`);
          }
        }
      } catch (pollError) {
        if (!active) {
  ...
  }, [entityLabel, fetchImport, importRecord?.id, onCompleted, step, toast]);
  ...
  const handleFileChange = async (file?: File) => {
    ...
    try {
      ...
      toast.danger(`Failed to analyze the ${entityLabel.toLowerCase()} import file.`);
    } finally {
      setBusy(false);
    }
  };
  ...
  const handlePreview = async () => {
    ...
    try {
      const response = await previewImport(importRecord.id, mapping);
      setPreview(response);
  ...
  const handleStart = async () => {
    ...
    try {
      const response = await startImport(importRecord.id);
      setImportRecord(response.import);
  ...
  const handleDownloadErrors = async () => {
    ...
    try {
      const blob = await downloadErrorReport(importRecord.id);
      saveAs(blob, `${templateBaseName.replace(/template/i, "errors")}.csv`);
    } catch (downloadError) {
  ...
  const handleDownloadTemplate = async (format: TemplateFormat) => {
    try {
      ...
      saveAs(blob, `${templateBaseName}.${format}`);
    } catch (downloadError) {
