...
export default function WorkspaceHeader<T>({
  ...
  // Derive contextual label from the URL — e.g. /admin/companies/123 → "Back to Companies"
  const backLabel = React.useMemo(() => {
    const parts = location.pathname.split("/").filter(Boolean);
    const listSegment = parts.length >= 2 ? parts[parts.length - 2] : parts[0];
    if (!listSegment || listSegment === "detail") {
      return parts.length >= 3 ? parts[parts.length - 3] : "Back";
    }
    return listSegment
      .split("-")
        ...
        <div className="card-body pt-9 pb-8">
          <div className="d-flex flex-wrap flex-sm-nowrap">
            ...
            <div className="flex-grow-1">
              ...
              <div className="d-flex justify-content-between align-items-start flex-wrap mb-5">
                <div>
                  <div className="d-flex align-items-center flex-wrap mb-2">
                    <h1 className="text-gray-900 fs-2hx fw-bold me-3 mb-0">{title}</h1>
                  ...
                  {subtitle && (
                    <div className="text-gray-600 fw-semibold fs-6">{subtitle}</div>
                  )}
                </div>
              ...
              {/* Metrics */}
              {config.metrics &&
                config.metrics.some((metric) => !metric.showIf || metric.showIf(data)) && (
