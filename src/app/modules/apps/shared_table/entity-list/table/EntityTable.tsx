...
const EntityTable = <T extends { id: string | number }>({
  data, columns, enableRowClick = false, getRowLink,
  selectedRows = new Set(), onRowSelect, onSelectAll, rowActions,
}: Props<T>) => {
  ...
  const handleRowClick = (row: T) => {
    ...
    navigate(getRowLink(row), {
      state: { data: row, columns: columns.map((c) => ({ key: c.accessor, label: c.Header })) },
    })
  }
  ...
  return (
    <KTCardBody>
      <div className="table-responsive border rounded">
        <table className="table align-middle table-bordered fs-4 g-6 table-row-gray-300" {...getTableProps()}>
          <thead>
            {headerGroups.map((hg) => {
              ...
              return (
                <tr key={key} {...rest}>
                  ...
                  {hg.headers.map((col) => {
                    ...
                    return (
                      <th key={key} {...rest} style={{ whiteSpace: "nowrap", fontWeight: 800, fontSize: "16px" }}>
                        {col.render("Header")}
                      </th>
                    )
                  })}
                  {hasActions && <th style={{ width: 120, textAlign: "center" }}>Action</th>}
                </tr>
              )
            })}
          </thead>
            ...
            {rows.length === 0 ? (
              ...
              return (
                <tr
                  ...
                  style={{
                    whiteSpace: "nowrap",
                    cursor: enableRowClick && getRowLink ? "pointer" : "default",
                  ...
                  {hasActions && (
                    <td onClick={(e) => e.stopPropagation()} style={{ textAlign: "center" }}>
                      <div className="dropdown">
                        <button className="btn btn-sm btn-light btn-active-light-primary" data-bs-toggle="dropdown">
                          Actions
                        </button>
