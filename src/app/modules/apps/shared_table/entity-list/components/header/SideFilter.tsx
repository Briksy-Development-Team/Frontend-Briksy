...
type FilterConfig =
  | { key: string; label: string; type: 'select'; options: string[] | { label: string; value: string | number }[] }
...
const SideFilter = ({ filters, onFilterChange }: Props) => {
  ...
  const setRange = (key: string, field: 'min' | 'max' | 'from' | 'to', val: string) =>
    setValues((prev) => ({
      ...prev,
      [key]: { ...(prev[key] as any), [field]: val === '' ? undefined : field === 'min' || field === 'max' ? Number(val) : val },
    }))
  ...
  return (
    <div className="card shadow-sm px-4">
          ...
          <div key={f.key} className="border-bottom">
            ...
            >
              <div className="fw-bold">
                {f.label}
                {getCount(f.key) > 0 && (
                  <span className="badge badge-light-primary ms-2">{getCount(f.key)}</span>
                )}
              </div>
              <KTIcon iconName={open === f.key ? 'minus' : 'plus'} className="fs-2" />
            </div>
                    ...
                    {f.options.map((opt) => {
                      ...
                      return (
                        <label key={String(value)} className="form-check mb-2">
                          <input
                            type="checkbox"
                            checked={((values[f.key] as (string | number)[]) || []).includes(value)}
      ...
      <div className="card-footer d-flex gap-2">
        <button className="btn btn-light w-100" onClick={() => { setValues({}); onFilterChange({}) }}>Reset</button>
        <button className="btn btn-primary w-100" onClick={() => onFilterChange(values)}>Apply</button>
      </div>
    </div>
  )
}
