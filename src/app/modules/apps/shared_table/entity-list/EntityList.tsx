...
const EntityList = <T extends { id: number | string }>({
  data, total, params, onParamsChange, columns,
  filtersConfig, enableRowClick, getRowLink,
  storageKey = "visibleColumns", addAction, rowActions,
}: Props<T>) => {
  ...
  const [visibleColumns, setVisibleColumns] = useState<string[]>(() => {
    try {
      ...
      return parsed.filter((key: string) => columns.some((c) => c.accessor === key))
    } catch {
      ...
      <div className="flex-grow-1" style={{ minWidth: 0 }}>
        <KTCard>
          <EntityHeader
            ...
            onSortChange={(config) => onParamsChange({
              ...params, sort: config.key, direction: config.direction, page: 1,
            })}
            addAction={addAction}
          />
      ...
      {filtersConfig && isMobile && showFilter && (
        ...
        >
          ...
          >
            <SideFilter
              filters={filtersConfig}
              onFilterChange={(f) => { onParamsChange({ ...params, filters: f, page: 1 }); setShowFilter(false) }}
            />
          </div>
        </div>
      )}
