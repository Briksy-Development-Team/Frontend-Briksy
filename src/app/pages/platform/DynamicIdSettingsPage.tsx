      ...
      {editing ? (
        <ModalShell title={editing.id ? "Edit Rule" : "New Rule"} onClose={() => setEditing(null)} onSubmit={submit} isValid={!!editing.entity_type}>
          <div className="row g-5">
            ...
            <div className="col-md-4"><label className="form-label">Padding</label><input className="form-control form-control-solid" type="number" min="0" onKeyDown={(e) => { if (e.key === "-" || e.key === "e") e.preventDefault(); }} value={editing.number_padding ?? 6} onChange={(e) => setEditing((current) => ({ ...current, number_padding: Number(e.target.value) }))} /></div>
            <div className="col-md-4"><label className="form-label">Start</label><input className="form-control form-control-solid" type="number" min="0" onKeyDown={(e) => { if (e.key === "-" || e.key === "e") e.preventDefault(); }} value={editing.starting_number ?? 1} onChange={(e) => setEditing((current) => ({ ...current, starting_number: Number(e.target.value) }))} /></div>
