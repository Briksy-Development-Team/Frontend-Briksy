...
export const WelcomeScreen = () => {
  ...
  return (
    <ScreenWrapper className="w-full px-[3rem] py-[2.5rem] flex flex-col items-center text-center gap-[1.5rem] max-w-[40.625rem] mx-auto text-[#342511]">
        ...
        {ACTIONS.map(({ icon: Icon, title, description }) => (
          <div key={title} className="flex items-start gap-[1rem] p-[1rem] rounded-[0.875rem] border border-[#EDE8E4] bg-white hover:bg-[#F8F4EE] transition-colors cursor-pointer">
            <div className="w-[2.5rem] h-[2.5rem] rounded-full bg-[#EEECE0] flex items-center justify-center shrink-0">
              <Icon size={20} className="text-[#342511]" />
            </div>
            <div className="flex flex-col gap-[0.25rem]">
              <h3 className="text-[0.875rem] font-medium text-[#342511]">{title}</h3>
              <p className="text-[0.75rem] text-[#7C5F42]">{description}</p>
            </div>
          </div>
        ))}
      </div>
