...
export default function FullListView({ 
  ...
  const getTitle = () => {
    const typeName = resultType === "property" ? "properties" : resultType === "comercial" ? "commercial properties" : resultType === "builder" ? "builders" : "professionals";
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between py-2">
        <h2 className="text-[1.5rem] font-medium text-[#342511] tracking-tight">
          {getTitle()}
        </h2>
      </div>
      ...
      {totalPages > 1 && (
        <div className="mt-8 flex items-center justify-center gap-2">
          <button 
            ...
            disabled={page === 1}
            className="flex items-center justify-center w-10 h-10 rounded-lg border border-[#E2CBB3] bg-white text-[#342511] disabled:opacity-50 hover:bg-[#F8F4EE] transition-colors"
          >
            <ChevronLeft size={20} />
          </button>
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
              <button
                ...
                className={`flex items-center justify-center w-10 h-10 rounded-lg transition-colors ${
                  page === p 
                    ? "bg-[#342511] text-white font-medium" 
                    : "border border-[#E2CBB3] bg-white text-[#342511] hover:bg-[#F8F4EE]"
                }`}
              >
                {p}
              </button>
