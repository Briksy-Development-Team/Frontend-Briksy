...
export default function CollectionsPage() {
  ...
  return <div className="w-full space-y-8">
    {createdMessage && <div role="status" className="fixed right-5 top-5 z-[120] rounded-2xl bg-primary-brown px-5 py-4 text-sm font-medium text-white shadow-xl">{createdMessage}</div>}
    ...
    <div className="rounded-2xl bg-white p-5 md:p-7">
      ...
      {loading ? <p className="text-primary-light-brown">Loading Collections...</p> : collections.length === 0 ? <div className="py-12 text-center text-primary-light-brown"><p>You haven't created any collections yet.</p></div> : <div className="grid gap-4 md:grid-cols-2">{collections.map((collection) => { const count = collection.items_count ?? collection.properties_count; return <div key={collection.id} className="rounded-2xl border border-[#EDE8E4] p-5"><div className="flex items-start justify-between gap-3"><Link to={`/profile/collections/${collection.id}`} className="min-w-0"><h2 className="truncate text-lg font-medium text-primary-brown">{collection.name}</h2><p className="mt-1 text-sm text-primary-light-brown">{count} {count === 1 ? "saved item" : "saved items"}</p></Link><div className="flex gap-2"><button type="button" aria-label="Rename Collection" onClick={() => { setEditing(collection); setName(collection.name); setComposerOpen(true); }}><Pencil size={17} /></button><button type="button" aria-label="Delete Collection" onClick={() => void remove(collection)}><Trash2 size={17} /></button></div></div><Link to={`/profile/collections/${collection.id}`} className="mt-5 block text-sm font-medium text-primary-brown">Open Collection →</Link></div>})}</div>}
    </div>
  </div>;
}
